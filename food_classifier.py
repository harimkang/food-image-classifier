...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-d", "--data_set", default="food-101", help="Select input path folder"
    )
    parser.add_argument(
        "-t", "--test_data", nargs="+", default=None, help="input test imgs path"
    )
    parser.add_argument("-b", "--batch_size", default=16, help="input train batch size")
            ...
            if model.load():
                if test_data_path:
                    for data_name in test_data_path:
                        model.prediction(img_path="examples/{}".format(data_name))
                else:
                    print(
