...
class Food101Test(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.model = Inception_v3(class_list=self.label, img_width=self.img_width, img_height=self.img_height,
                                  batch_size=16)
    ...
    def test_prediction_food_3_images(self):
        if self.model.load():
            ...
            result2 = self.model.prediction(
                img_path='examples/pizza.jpg', show=False)
            self.assert_(result2)
            result3 = self.model.prediction(
                img_path='examples/samosa.jpg', show=False)
